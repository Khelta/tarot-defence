...
func _process(delta):
	for enemy_path in level_path.get_children():
...
func calculate_progress_ratio_speed(enemy: BaseEnemy) -> float:
	var path_length: float = level_path.curve.get_baked_length()
	var time_for_path = path_length * level_path_speed
